...
import { FlaskConical, Save, Plus, Trash2, CheckCircle2, AlertCircle, Loader2, FileCheck, Users } from 'lucide-react';
...
export const MixingEntryForm: React.FC<{ onSuccess?: () => void; productionReferences?: ProductionReferenceSelection; productionGenealogy?: ProductionGenealogySelection }> = ({ onSuccess, productionReferences, productionGenealogy }) => {
  ...
  const tr = {
    ...
    savedSuccess: language === 'ar' ? 'تم حفظ سجل الخلط والتجهيز بنجاح.' : 'Mixing record saved successfully.',
    saveFailed: language === 'ar' ? 'فشل الحفظ.' : 'Save failed.',
    stageTitle: language === 'ar' ? 'تسجيل إنتاج: الخلط والتجهيز (Mixing Stage)' : 'Production Entry: Mixing Stage',
    stageSubtitle: language === 'ar' ? 'تجهيز وتجنيس الخلطات الحرارية ونسب إضافة الخامات' : 'Thermal mix preparation & homogenization and material addition ratios',
    stage6: language === 'ar' ? 'المرحلة 6' : 'Stage 6',
    mixDate: language === 'ar' ? 'تاريخ الخلط' : 'Mixing Date',
    linkedProduct: language === 'ar' ? 'ربط بمنتج من البيانات الأساسية (اختياري)' : 'Link to a Master Data Product (optional)',
    searchProductPlaceholder: language === 'ar' ? 'ابحث لتعبئة اسم وكود الخلطة تلقائيًا...' : 'Search to auto-fill the mix name/code...',
    mixName: language === 'ar' ? 'اسم الخلطة' : 'Mix Name',
    mixCode: language === 'ar' ? 'كود الخلطة' : 'Mix Code',
    preparedQuantity: language === 'ar' ? 'الكمية المجهزة (طن)' : 'Prepared Quantity (t)',
    wasteLoss: language === 'ar' ? 'الهالك / الفاقد (طن)' : 'Waste / Loss (t)',
    homogeneityEfficiency: language === 'ar' ? 'نسبة التجانس والكفاءة' : 'Homogeneity & Efficiency Rate',
    mixMaterials: (n: number) => language === 'ar' ? `خامات الخلطة (${n})` : `Mix Materials (${n})`,
  ...
  const [mixProductCode, setMixProductCode] = useState('');
  ...
  const resetForm = () => {
    ...
    setMixProductName('');
    setMixProductCode('');
    setBatchNumber(`MIX-${Date.now().toString().slice(-4)}`);
  ...
  const handleSubmit = async (e: React.FormEvent, status: 'SUBMITTED' | 'DRAFT' = 'SUBMITTED') => {
    e.preventDefault();
    if (status === 'SUBMITTED') {
      const warnings = evaluateStageWarnings('mixing', { production: productionQuantity, waste: wasteQuantity }, language);
  ...
  const performSave = async (status: 'SUBMITTED' | 'DRAFT', overriddenWarnings: ValidationResult[]) => {
    ...
    try {
      ...
      const newRecordId = await createStageRecord('mixing', await attachProductionReferences('mixing', {
        date,
        mixProductName,
        mixProductCode,
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e, 'SUBMITTED')} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-7 space-y-6" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-3">
        ...
        <span className="self-start sm:self-auto text-xs font-bold px-3 py-1 bg-indigo-100 text-indigo-800 rounded-lg">{tr.stage6}</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1.5">{tr.mixDate} *</label>
          <input type="date" required value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500/20 outline-none" />
        </div>
        <SmartEntitySelect
          id="mix-linked-product-select"
          ...
          onChange={(id, opt) => {
            ...
            if (opt) {
              setMixProductName(opt.name);
              setMixProductCode(opt.code);
            }
          }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1.5">{tr.mixName} *</label>
          <input type="text" required value={mixProductName} onChange={(e) => setMixProductName(e.target.value)} className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-300 rounded-xl font-bold" />
        </div>
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1.5">{tr.mixCode}</label>
          <input type="text" value={mixProductCode} onChange={(e) => setMixProductCode(e.target.value)} className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-300 rounded-xl font-mono" />
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-indigo-50/40 p-4 rounded-xl border border-indigo-200">
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">{tr.preparedQuantity}</label>
        ...
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">{tr.wasteLoss}</label>
        ...
        <div>
          ...
          <div className="w-full px-3.5 py-2 text-sm bg-white border border-slate-300 rounded-xl font-bold text-emerald-600">{yieldPercentage}%</div>
        </div>
      </div>
      ...
      <div>
        ...
        <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full px-3.5 py-2.5 text-sm bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500/20 outline-none" />
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
        ...
        <button type="submit" disabled={isSubmitting} className="flex items-center gap-2 px-6 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-all cursor-pointer">
