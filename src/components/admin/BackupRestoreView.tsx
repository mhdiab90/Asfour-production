...
import { generateRestorePreview, executeSafeRestore } from '../../services/restoreService';
...
export const BackupRestoreView: React.FC<BackupRestoreViewProps> = ({ initialTab = 'backups' }) => {
  ...
  const loadBackups = async () => {
    ...
    try {
      ...
      setFeedback({ type: 'error', message: `فشل في تحميل سجلات النسخ الاحتياطي: ${err.message}` });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCreateBackup = async () => {
    ...
    setBackupPercent(0);
    setBackupProgressMsg('جاري بدء النسخ الاحتياطي...');
    ...
    try {
      ...
      setFeedback({
        type: 'success',
        message: `تم إنشاء النسخة الاحتياطية (${newBackup.backupId}) بنجاح! تم حفظ ${newBackup.totalRecords} سجل.`
      });
      await loadBackups();
    } catch (err: any) {
      setFeedback({ type: 'error', message: `فشل إنشاء النسخة الاحتياطية: ${err.message}` });
    } finally {
  ...
  const handleExecuteRestore = async () => {
    ...
    if (confirmationCode.trim().toUpperCase() !== 'RESTORE ASFOUR DATA') {
      setFeedback({ type: 'error', message: 'يرجى كتابة رمز التأكيد بشكل صحيح (RESTORE ASFOUR DATA) لإتمام العملية بأمان.' });
      return;
    }

    setIsRestoring(true);
    setRestorePercent(0);
    setRestoreProgressMsg('جاري بدء الاستعادة الآمنة...');
    ...
    try {
      ...
      if (result.success) {
        setFeedback({
          type: 'success',
          message: `تمت استعادة ${result.totalRestored} سجل بنجاح! تم إنشاء نقطة أمان احتياطية برقم: ${result.safetyBackupId || 'N/A'}`
        });
      } else {
        setFeedback({
          type: 'error',
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setFeedback({ type: 'success', message: `تم حذف النسخة الاحتياطية (${backupToDelete.backupId}) بنجاح.` });
  ...
  return (
    <div className="space-y-6 pb-12" dir="rtl">
      {/* Top Banner / Feedback */}
      {feedback && (
        <div className={`p-4 rounded-2xl flex items-center justify-between gap-3 text-sm shadow-md animate-in fade-in ${
          ...
          <div className="flex items-center gap-2">
            {feedback.type === 'success' && <CheckCircle2 className="w-5 h-5 shrink-0 text-emerald-400" />}
            {feedback.type === 'error' && <AlertTriangle className="w-5 h-5 shrink-0 text-rose-400" />}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                إدارة النسخ الاحتياطية الكاملة، الحماية من الكوارث، ومعاينة واستعادة البيانات بصورة آمنة
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-amber-500 hover:bg-amber-600 active:scale-95 text-slate-950 font-bold px-5 py-3 rounded-2xl text-sm flex items-center gap-2 shadow-lg shadow-amber-500/10 transition"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-slate-800">
          ...
          <div className="bg-slate-800/50 border border-slate-800 p-4 rounded-2xl">
            ...
            <p className="text-[11px] text-slate-400 mt-1">عبر كافة مجموعات المصنع</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="البحث برقم النسخة، الملاحظات، أو النوع (MANUAL, PRE_IMPORT, SAFETY_CHECKPOINT)..."
                    ...
                    <tr key={backup.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3.5 px-4">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            onClick={() => exportBackupToFile(backup)}
                            className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition"
                            title="تحميل ملف النسخة الاحتياطية (JSON)"
                          >
                          ...
                          <button
                            ...
                            className="p-2 rounded-xl bg-amber-500/20 hover:bg-amber-500 text-amber-300 hover:text-slate-950 border border-amber-500/30 transition"
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              تتيح لك هذه الواجهة استعادة بيانات النظام بأعلى معايير الأمان المعتمدة. تتم الاستعادة عبر المراحل التالية:
            </p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
            <h4 className="text-sm font-bold text-white mb-4">اختر نسخة احتياطية لبدء المعاينة والاستعادة:</h4>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl text-white">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-lg">إنشاء نسخة احتياطية جديدة</h3>
                <p className="text-xs text-slate-400">سيتم حفظ نسخة كاملة من كافة المجموعات الأساسية وسجلات الإنتاج</p>
              </div>
            </div>
            ...
            <div className="space-y-4 my-4">
              <div>
                ...
                <input
                  ...
                  placeholder="مثال: نسخة احتياطية قبل إغلاق حسابات الشهر..."
              ...
              <div className="bg-slate-800/60 p-3.5 rounded-2xl border border-slate-700/50 text-xs text-slate-300 space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-blue-400">v{CURRENT_APP_VERSION.version}</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
              ...
              <button
                ...
                disabled={isCreatingBackup}
                className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold transition flex items-center gap-2 shadow-lg shadow-amber-500/10 disabled:opacity-50"
              >
                <Plus className="w-4 h-4" />
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-2xl w-full p-6 shadow-2xl text-white my-8 max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800 shrink-0">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-bold transition flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-amber-500/10"
              >
                <RotateCcw className="w-4 h-4" />
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" dir="rtl">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl text-white">
            <div className="flex items-center gap-3 text-rose-400 mb-3">
              ...
              <div>
                <h3 className="font-bold text-lg text-white">حذف النسخة الاحتياطية</h3>
                <p className="text-xs text-slate-400 font-mono">{backupToDelete.backupId}</p>
              </div>
            </div>

            <p className="text-sm text-slate-300 my-4">
              هل أنت متأكد من رغبتك في حذف هذه النسخة الاحتياطية؟ لن تتمكن من استعادة البيانات من هذه النقطة بعد الحذف.
            </p>
